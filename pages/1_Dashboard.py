...
with st.sidebar:
    ...
    .mi{font-family:'Material Symbols Outlined';font-variation-settings:'FILL' 0,'wght' 400,'GRAD' 0,'opsz' 24;
        line-height:1;display:inline-block;vertical-align:middle;}

    /* ── Nav item wrapper: relative so button can overlay it ── */
    .nav-item-wrap{position:relative;margin-bottom:2px;}

    /* ── The visible HTML nav row ── */
    .nav-row{
    ...
    .nav-row.active{background:rgba(37,99,235,0.08);color:#2563EB;font-weight:700;}
    .nav-row.inactive{color:#64748b;font-weight:500;}
    .nav-row .mi{font-size:20px;}
    ...
    [data-testid="stSidebar"] .stButton>button{
      ...
      font-size:0!important; /* hide any text bleed */
      z-index:2!important;
    }
    [data-testid="stSidebar"] .stButton>button:hover~div .nav-row,
    [data-testid="stSidebar"] .stButton>button:hover{
      background:rgba(0,0,0,0)!important;
    }
    /* hover effect on the wrapper via JS-free CSS sibling trick */
    .nav-item-wrap:hover .nav-row.inactive{
      background:#f8fafc!important;color:#0f172a!important;
    }

    /* Logout button — keep visible */
    [data-testid="stSidebar"] .stButton.logout-btn>button{
      position:static!important;
      width:100%!important;height:auto!important;
      background:#fff!important;color:#475569!important;
      border:1px solid #e2e8f0!important;
      font-size:0.875rem!important;color:#475569!important;
    ...
    # ── Logo ──
    st.markdown(f"""
    <div style="padding:22px 20px 18px;display:flex;align-items:center;gap:10px;border-bottom:1px solid #f1f5f9;">
      ...
      <span style="font-size:1.1rem;font-weight:800;color:#0f172a;font-family:'DM Sans',sans-serif;letter-spacing:-0.3px;">CareerSync</span>
    </div>
    <div style="padding:12px 10px;">
    """, unsafe_allow_html=True)

    # ── Nav items: HTML label + invisible Streamlit button overlay ──
    nav_items = [
    ...
    for icon, label, path, is_active in nav_items:
        ...
        # The visible nav row (pointer-events:none — button handles clicks)
        st.markdown(f"""
        <div class="nav-item-wrap">
          <div class="nav-row {row_class}">
            <span class="mi" style="font-variation-settings:'FILL' {fill},'wght' 400,'GRAD' 0,'opsz' 24;">{icon}</span>
            {label}
          </div>
        """, unsafe_allow_html=True)

        if not is_active:
            # Invisible button overlaid on top — no label shown (font-size:0)
            if st.button("​", key=f"nav_{label}"):   # zero-width space as label
                try:    st.switch_page(path)
                except: st.info(f"{label} coming soon!")

        st.markdown("</div>", unsafe_allow_html=True)   # close nav-item-wrap

    st.markdown("</div>", unsafe_allow_html=True)  # close nav padding div
    ...
    <div style="padding:12px 10px 14px;">
      <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:10px;background:#f8fafc;">
        <div style="width:38px;height:38px;border-radius:50%;background:#e0e7ff;display:flex;align-items:center;
    ...
    # Logout — use a container div so CSS .logout-btn selector works
