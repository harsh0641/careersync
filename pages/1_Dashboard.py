...
with st.sidebar:
    ...
    <div style="display:flex;align-items:center;gap:8px;padding:20px 4px 24px;">
      ...
      <span style="font-size:1.2rem;font-weight:700;color:#0f172a;">CareerSync</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    for icon, label, page_path in pages:
        is_active = "1_Dashboard" in page_path
        bg    = "background:rgba(37,99,235,0.08);color:#2563EB;" if is_active else "background:transparent;color:#475569;"
        st.markdown(f"""
        <div style="{bg}display:flex;align-items:center;gap:10px;padding:10px 12px;
             border-radius:8px;margin-bottom:2px;font-weight:{'700' if is_active else '500'};
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:8px 4px 12px;">
      ...
      <div style="min-width:0;">
        ...
        <div style="font-size:0.72rem;color:#64748b;
                    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{email_disp}</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown(
    ...
    f"Welcome back, {user.get('name','').split()[0] if user.get('name') else 'there'}! "
    f"Here's your job search overview.</p>",
    unsafe_allow_html=True,
)
...
BADGE_MAP = {"Applied":"b-applied","Interview":"b-interview","Offer":"b-offer","Rejected":"b-rejected"}


def build_table(rows_df):
    thead = ('<div class="cs-wrap"><table class="cs-table"><thead><tr>'
    ...
    for _, r in rows_df.iterrows():
        ...
        email_cell = (f'<span class="email-txt">{email}</span>' if email
        ...
        rows += (f"<tr>"
                 ...
                 f"<td>{rec_cell}</td><td>{email_cell}</td></tr>")
...
def build_credits_panel():
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:4px;">
    <div><div style="font-size:.875rem;font-weight:700;color:#0f172a;">{name}</div>
    <div style="font-size:.72rem;color:#94a3b8;margin-top:1px;">{sub}</div></div>
    <span style="font-size:.78rem;font-weight:700;color:#334155;">{used:,} / {total:,}</span>
  </div>
  <div class="credit-bar-bg"><div class="credit-bar-fill" style="background:{color};width:{pct}%;"></div></div>
</div>"""
    return f"""<div class="credit-wrap">
  <div style="font-size:1rem;font-weight:700;color:#0f172a;margin-bottom:20px;">Credit Usage</div>
  {items}
  ...
  <button style="width:100%;padding:10px;border-radius:8px;border:1px solid #e2e8f0;
    background:#fff;font-size:.875rem;font-weight:700;color:#0f172a;cursor:pointer;">Upgrade Plan</button>
</div>
<div class="pro-tip">
...
with col_left:
    st.markdown(
        f"<h3 style='font-size:1.05rem;font-weight:700;color:#0f172a;margin-bottom:12px;'>"
    ...
    if total_pages > 1:
        ...
        def page_slots(cur, total):
            ...
            result=[0]; lo,hi=max(1,cur-2),min(total-2,cur+2)
        ...
        btn_cols = st.columns(2+len(slots))
        with btn_cols[0]:
        ...
        for i,slot in enumerate(slots):
            with btn_cols[i+1]:
                if slot is None:
                    ...
                    if st.button(str(slot+1),key=f"pg_{slot}",type="primary" if is_cur else "secondary",use_container_width=True):
...
if not rows_for_email:
    ...
    with ce1:
        sel_label = st.selectbox("Select Recruiter",[r["label"] for r in rows_for_email],key="email_rec_sel",label_visibility="collapsed")
        ...
        with st.spinner("✨ Writing your cold email..."):
            try:
                ...
                resp = _req.post(
                    ...
                    json={"model":"llama-3.1-8b-instant","messages":[{"role":"user","content":prompt}],"max_tokens":420,"temperature":0.72},
                    timeout=20,
                )
    ...
    if st.session_state.get("ai_email_subj") or st.session_state.get("ai_email_body"):
        ...
        with ca:
            if st.button("📋 Copy Email",key="copy_email_btn"):
                st.success("Use Ctrl+A in the body field!", icon="✅")
        with cb:
            to_addr = st.session_state.get("ai_email_to","")
            mailto  = (f"mailto:{to_addr}?subject={subj_edited.replace(' ','%20')}"
...
# MANAGE APPLICATIONS
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("<h3 style='font-size:1.1rem;font-weight:700;color:#0f172a;margin-bottom:4px;'>⚙️ Manage Applications</h3>",unsafe_allow_html=True)
        ...
        with col_b:
            with st.expander("✏️ Override manually"):
                ...
                if st.button("💾 Save Override",key="t2_save"):
                    update_recruiter_info(app_id2,me,mn,mt_val,ml); st.success("Saved!"); st.rerun()
...
with tab3:
    ...
    if st.button("➕ Add Application",key="m_add",type="primary"):
        if mc and mt:
            ...
            upsert_application(mc,mt,str(md),"Manually added",info.get("recruiter_email",""),info.get("recruiter_name",""),info.get("recruiter_title",""),info.get("linkedin_url",""))
